...
fn cd_missing_has_no_os_error_suffix_and_prologue() {
    ...
    assert!(
        e.contains(": line 1: cd: /no/such_xyz: No such file or directory\n"),
        "stderr: {e:?}"
    );
    assert!(!e.contains("os error"), "leaked Rust suffix: {e:?}");
    assert!(
        !e.starts_with("huck:"),
        "file mode should not use huck: prologue: {e:?}"
    );
}
...
fn redirect_read_missing_has_prologue_no_suffix() {
    ...
    assert!(
        e.contains(": line 1: /no/such_xyz: No such file or directory\n"),
        "stderr: {e:?}"
    );
    assert!(!e.contains("os error"), "leaked Rust suffix: {e:?}");
}
...
fn redirect_write_to_directory_is_a_directory() {
    ...
    assert!(
        e.contains(": line 1: /etc: Is a directory\n"),
        "stderr: {e:?}"
    );
    assert!(!e.contains("os error"), "leaked Rust suffix: {e:?}");
}
...
fn source_not_found_matches_bash() {
    ...
    assert!(
        !e.contains(".: /no/such_xyz"),
        "should not use the `.:` prefix for not-found: {e:?}"
    );
}
