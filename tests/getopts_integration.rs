...
fn simple_loop_parses_options() {
    let (out, _e, _c) = run("set -- -a -b val -c arg1\n\
...
fn no_args_uses_positional_params() {
    let (out, _e, _c) = run("f() { while getopts \"x\" o; do echo \"$o\"; done; }\n\
         f -x -x\n");
    assert_eq!(out, "x\nx\n", "out: {out}");
}
...
fn usage_error_drops_huck_prefix_and_fixes_arg_ellipsis() {
    ...
    assert_eq!(
        e, "getopts: usage: getopts optstring name [arg ...]\n",
        "stderr: {e:?}"
    );
    assert_eq!(c, 2);
}
...
fn invalid_option_to_getopts_itself_is_rejected() {
    ...
    assert!(
        e.contains("getopts: usage: getopts optstring name [arg ...]"),
        "stderr: {e:?}"
    );
    assert_eq!(o, "rc=2\n", "stdout: {o:?}");
...
fn invalid_name_still_binds_optind() {
    ...
    assert_eq!(o, "oi=2\n", "stdout: {o:?}");
    assert!(
        e.contains("`bad-name': not a valid identifier"),
        "stderr: {e:?}"
    );
}
