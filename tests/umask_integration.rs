...
fn octal_out_of_range_keeps_mask() {
    ...
    assert!(
        e.contains("umask: 09: octal number out of range"),
        "stderr: {e}"
    );
...
fn invalid_symbolic_character() {
    ...
    assert!(
        e.contains("umask: `u': invalid symbolic mode character"),
        "stderr: {e}"
    );
}
...
fn invalid_symbolic_operator() {
    ...
    assert!(
        e.contains("umask: `:': invalid symbolic mode operator"),
        "stderr: {e}"
    );
}
...
fn invalid_option() {
    ...
    assert!(
        e.contains("umask: usage: umask [-p] [-S] [mode]"),
        "stderr: {e}"
    );
}
