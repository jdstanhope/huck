...
fn case_and_dbracket_membership() {
    assert_eq!(
        run("case \" \" in [[:space:]]) echo SP;; *) echo no;; esac\n"),
        "SP\n"
    );
    assert_eq!(
        run("case \"5\" in [[:space:]]) echo SP;; *) echo no;; esac\n"),
        "no\n"
    );
    assert_eq!(
        run("[[ \"x\" == [[:alpha:]] ]] && echo Y || echo N\n"),
        "Y\n"
    );
    assert_eq!(
        run("[[ \"5\" == [[:alpha:]] ]] && echo Y || echo N\n"),
        "N\n"
    );
}
#[test]
