...
#[test]
fn two_input_process_subs() {
    assert_eq!(huck("cat <(echo a) <(echo b)"), "a\nb\n");
}
#[test]
fn redirect_source_process_sub() {
    assert_eq!(huck("wc -c < <(printf hello)").trim(), "5");
}
#[test]
fn while_read_from_process_sub() {
    assert_eq!(
        huck("while read x; do echo \"[$x]\"; done < <(seq 3)"),
        "[1]\n[2]\n[3]\n"
    );
}
#[test]
fn output_process_sub_tee() {
    assert_eq!(huck("printf 'foo\\n' | tee >(cat) >/dev/null"), "foo\n");
}
#[test]
fn nested_process_sub() {
    assert_eq!(huck("cat <(cat <(echo deep))"), "deep\n");
}
#[test]
fn quoted_is_literal() {
    assert_eq!(huck("echo \"<(echo hi)\""), "<(echo hi)\n");
}
#[test]
fn procsub_arg_in_pipeline() {
...
#[test]
fn procsub_redirect_in_pipeline() {
...
#[test]
fn pipeline_procsub_no_zombies() {
    // many pipeline procsubs must not leave zombies (the shell reaps them)
    let out = huck(
        "for i in 1 2 3 4 5; do cat <(echo x) | cat >/dev/null; done; ps -o stat= --ppid $$ 2>/dev/null | grep -c defunct || echo 0",
    );
