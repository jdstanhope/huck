...
fn shopt_set_query_roundtrip() {
    ...
    assert_eq!(
        run("shopt -s nullglob; shopt -q nullglob; echo $?\n").0,
        "0\n"
    );
}
...
fn shopt_inert_option_tracks() {
    ...
    assert_eq!(
        run("shopt -s extglob; shopt -q extglob; echo $?\n").0,
        "0\n"
    );
}
...
fn shopt_query_prints_state() {
    assert_eq!(
        run("shopt -s dotglob; shopt dotglob\n").0,
        "dotglob        \ton\n"
    );
}
...
fn shopt_p_with_name_prints_reinput() {
    ...
    assert_eq!(
        run("shopt -s nullglob; shopt -p nullglob\n").0,
        "shopt -s nullglob\n"
    );
}
...
fn nullglob_no_match_expands_empty() {
    ...
    assert_eq!(
        run_in_dir(&["a.txt"], "shopt -s nullglob; echo no*match\n").0,
        "\n"
    );
}
...
fn dotglob_includes_dotfiles() {
    ...
    assert_eq!(
        run_in_dir(&["a", ".hidden"], "shopt -s dotglob; echo *\n").0,
        ".hidden a\n"
    );
}
...
fn nocaseglob_matches_case_insensitively() {
    ...
    assert_eq!(
        run_in_dir(&["Abc.txt"], "shopt -s nocaseglob; echo a*\n").0,
        "Abc.txt\n"
    );
}
...
fn nocasematch_double_bracket_eq() {
    ...
    assert_eq!(
        run("shopt -s nocasematch; [[ ABC == abc ]] && echo m || echo n\n").0,
        "m\n"
    );
}
...
fn nocasematch_case_statement() {
    ...
    assert_eq!(
        run("shopt -s nocasematch; case ABC in abc) echo m;; *) echo n;; esac\n").0,
        "m\n"
    );
}
