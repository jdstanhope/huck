...
- **L-61: `func` bash-test category — last residual blocker is FUNCNEST** — `[deferred]`, medium. The `func` category was fully re-measured in v221; its original blockers have been cleared across v221–v223 (v221: prefix-assignment leak across function calls; v222: redirected-brace-body + nested-`function`-keyword reconstruction; v223: `declare -xF` export filter/format + FUNCNAME write protection). ONE blocker now remains — fixing it flips `func` to PASS: **`FUNCNEST` is not enforced** — huck ignores the `FUNCNEST` variable's recursion-depth limit entirely; worse, deep unbounded recursion overflows the Rust call stack and aborts the process (`fatal runtime error: stack overflow`) instead of bash's graceful `maximum function nesting level exceeded (N)` + return. The stack-overflow is a real robustness bug independent of the category.
