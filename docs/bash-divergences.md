...
| Low-impact (Tier 4) | 46 | Open edge cases / cosmetic divergences (`[low]`/`[intentional]`/`[deferred]`). |
...
Note: `Shell::error_prefix(cmd: Option<&str>) -> String` (v216) is the bash-compatible error-prologue foundation (`<name>: [line N: ][cmd: ]`). Converted so far: arith (v216, first site); the **getopts** error sites and the **generic readonly-assignment error** (v227 — `assign()` + the `run_assignment_list` early check, which together flipped the `getopts` bash-test category); the **command-not-found** message (v228 — `run_subprocess` spawn-NotFound site now emits `<src>: line N: <name>: command not found`, name before the phrase — a broad shrink across alias/builtins/execscript/errors, no flip); the **file-IO error sites** (v229 — `cd` via `error_prefix(Some("cd"))`, all 26 redirect-open sites via the single `redir_open_error` helper using `error_prefix(None)`, and the `source`/`.` builtin's four error forms via `error_prefix(None)`/`error_prefix(Some("."))` — broad shrink across alias/execscript/errors/dirstack/builtins, no flip); the **three new v230 builtins** `umask`/`ulimit`/`enable` emit their diagnostics via `error_prefix(Some("umask"|"ulimit"|"enable"))` (bare usage lines) — so they are born prologue-correct (the `umask` errors in the `errors` category match byte-for-byte). The remaining ~400 `huck:`-prefixed sites (other builtins' usage/readonly errors, parser, and the non-file-IO io error sites — pwd/echo/read/mapfile/jobs/history/pipe/fork/heredoc/expand, which v229 left `huck:`-prologued) are still staged across future iterations. **v227 measured all 11 prologue-touched categories first and confirmed the prologue flips ZERO categories alone — each has independent residual blockers (command-not-found word order — done v228; Rust `io::Error` text leakage — done v229; unimplemented builtins, `set -o` gaps).** So the prologue is converted per-category, where it is a category's LAST blocker (getopts was the smallest such residual), not in one shell-wide sweep.

The Rust `io::Error` text leakage (the ` (os error N)` Display suffix and the synthesized "stream did not contain valid UTF-8") was resolved shell-wide in v229: every io::Error formatting site now renders through `crate::bash_io_error(&e)` (bare strerror, suffix stripped; synthesized non-errno errors keep their Display), so message BODIES match bash's `strerror` / `cannot execute binary file` everywhere — independent of whether the site's PROLOGUE is converted yet.
...
- **L-68: residual umask/ulimit/enable builtin edges (post-v230)** — `[deferred]`, low (v230). v230 shipped `umask` (octal+symbolic, prologue errors), `ulimit` (full Linux resource table, real `get`/`setrlimit`), `enable` (a `disabled_builtins` set toggled via `enable -n`/`enable`, consulted by `builtin_active` in dispatch + `type`/`command -v`), and the `times` POSIX special builtin (so `enable -ps` lists it). Narrow residuals, none exercised by a currently-gating category (the v230 deliverable was a broad shrink — the `command not found` clusters left `builtins`/`errors`/`procsub` — with NO category flip): (a) **`enable -f file` / `enable -d` (dynamically-loaded builtins)** are rejected as `invalid option`; huck has no loadable-builtin support (the `loadable` bash-test category is in known-skips), so dynamic loading is out of scope. (b) **`ulimit -a` omits `-p` (pipe size) and `-R` (RLIMIT_RTTIME), and its column labels are not byte-verified** — `-a` absolute values are environment-specific and deliberately NOT harness-pinned; only the env-independent round-trips and error forms are checked by `ulimit_diff_check.sh`. (c) **`ulimit` value `saturating_mul` overflow** — a set value large enough that `value × multiplier` saturates `u64` is written as `RLIM_INFINITY` (unlimited) rather than erroring; only reachable with astronomically large inputs the kernel would reject anyway. (d) **the declaration-command path (`declare`/`local`/`export`/etc. via `is_declaration_command`) is not disable-aware** — `enable -n declare; declare x=1` still runs the declaration builtin, because that path is dispatched before the `builtin_active` check; rare and untested. The common forms (octal/symbolic umask, `ulimit -n`/`-c` round-trips, `enable -n`/`enable`/`enable -ps` listing) are byte-identical to bash (`umask_diff_check.sh` 10/10, `ulimit_diff_check.sh` 5/5, `enable_diff_check.sh` 6/6).

- **L-69: residual `expand_aliases` (non-interactive) edges (post-v232)** — `[deferred]`, low (v231/v232). v231 wired the existing alias expander into the non-interactive file/`source` seam (`run_sourced_contents_in_sinks`), honoring `shopt expand_aliases` in scripts via a provenance map (offsets stay anchored to the original source) + an `alias_generation` re-tokenize for def-then-use timing. **v232** then made the expander *command-position-aware*: `alias_expand::Expander` replaced the single `next_eligible` bool with reserved-word recognition + a `case`/`for`/`[[ ]]` context stack + function-definition `name()` header detection (`last_was_open_paren`). v232 RESOLVED the v231 over-expansion regression — a `case`-pattern word after `|` (e.g. `ls`) being wrongly alias-expanded, which broke `source ~/.bashrc` / nvm `bash_completion`, including the real `name() { … case … }` function-body shape — and fixed the opposite latent bug (missed expansions after `then`/`do`/`{`). This is a **major shrink** of the `alias` bash-test category but did NOT flip it; residuals remain, all now in the *under*-expansion direction (never the parse-breaking over-expansion): (a) **redirect-prefix command position** — a command word that follows a redirection (`< file foo`, `>out cmd`) is NOT alias-expanded, because op-eligibility in `feed_op` treats a redirect operator as ending command position; bash expands it. A bash-exact fix needs redirect-prefix-aware eligibility (a leading redirection + target should not consume command position). (b) **`-c` command-string mode is not wired** — `huck -c 'shopt -s expand_aliases; alias x=…; x'` does not expand, because the `-c` path (`run_cmdstring`) is separate from `run_sourced_contents_in_sinks`; the runner gates the `alias` category through file mode, so this is not category-blocking but is a real gap. (c) an **alias defined AND first used within the same logical unit** (`alias x=…; x` on one physical line / one compound) is not expanded — documented edge-limit; a bash-faithful fix needs a push-back lexer. (d) two test-specific residuals in the `alias` category (`alias2.sub` `$TMPDIR`/permission infra, `alias4.sub` unterminated-quote parse) are unrelated to expansion. (e) **v232 minor under-expansions** (from final review): the parenless `function NAME { … }` body, the `coproc NAME { … }` body, and `time -p cmd`/`time --` flag forms do not put the following command word at command position, so an alias there is not expanded. All low-frequency, all under-expansion. The common def-on-one-line-use-on-a-later-line patterns, trailing-space continuation, redefine, unalias, case patterns, reserved-word slots, for-lists, and `[[ ]]` interiors are byte-identical to bash (`alias_expand_diff_check.sh` 7/7, `alias_case_diff_check.sh` 9/9), and `set -v` echoes the RAW pre-expansion line. v231 also fixed `source` path resolution (no-slash `$PATH`+CWD fallback gated on `shopt sourcepath`) and device-file acceptance (`/dev/null`, `/dev/stdin`, fifos, procsub `/dev/fd/N` — was rejected by `Path::is_file()`); both are byte-identical to bash (`source_device_diff_check.sh` 6/6) and shrink `builtins` (which keeps ~10 other blockers — no flip).
