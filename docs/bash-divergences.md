...
- **M-128: `declare -n` / `local -n` namerefs** — `[deferred]` medium (v158 follow-on; the third remaining declaration attribute after `-l`/`-u` shipped in v158). `declare -n ref=target` makes `ref` a NAME REFERENCE to `target`: reads (`$ref`, `${ref}`), writes (`ref=v` sets `target`), `unset ref` (vs `unset -n ref`), `${!ref}` (special: yields the target NAME), `for ref in …`, `read ref`, and `[[ -v ref ]]` all indirect through `target`, with circular-reference detection and array-element targets (`declare -n r=arr[i]`). huck currently errors `declare: -n: not yet implemented in this version` (the `b'n'` flag arm in all four declaration builtins still rejects). Substantial: touches variable resolution across expansion/assignment/unset/loop/read, so scoped as its own iteration (now v160 — v159 was the assignment-path-unification prerequisite). The WRITE side is now seam-ready: v159 routed every assignment through one `Shell::assign(dest, op, source)` chokepoint with an identity `resolve_assign_target(dest) -> dest` nameref seam (the ONE place to rewrite the destination, incl. `Whole(r)`→`Element{arr,Index}`), so nameref WRITES become a single-function change. The READ side (`$ref`, `${!ref}`, `[[ -v ref ]]`, `for ref`) and circular-ref detection still need their own hooks. The v158 `Variable` attribute model (`case_fold` field + `set_case_fold`) is the template for how the nameref attribute attaches.
