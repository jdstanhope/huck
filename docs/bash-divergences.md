...
- **Severity**: low
- **huck**: four residual differences from bash's xtrace output, none of which affect the diagnostic value (every executed command still prints one trace line). (a) **Finer compound traces not emitted**: bash separately traces the `for`-iteration variable set on each loop pass (`+ for i in 1 2`), the `case` word (`+ case x in`), and the `[[ ]]` / `(( ))` test/arith conditions WITH expansions applied (`+ [[ 1 -gt 0 ]]`, `+ ((  1 + 1  ))`); huck emits NOTHING for these compound headers (it traces only the inner simple commands). Re-confirmed in the wild by the v197 RUN_HUCK_DIFF runtime-sweep triage (e.g. a `[[ '' =~ … ]]` in jackc/pgx's `setup_test.bash`). Fixing it is feature-sized — needs an expansion-aware per-form source reconstructor emitted in each compound `run_*` (matching bash's whitespace quirks + per-iteration `for` tracing) — so it warrants its own iteration, not a bolt-on. (b) **Decl-RHS-with-command-substitution edge**: a `local`/`declare`/`export` RHS that contains a command substitution (e.g. `local x=$(cmd)`) is traced with the EXPANDED value, which re-runs the command substitution for the trace (double-execution) — rare. (c) Per M-90, `2>/dev/null` does NOT suppress the trace (builtin/executor stderr ignores `2>` — consistent with M-90). (d) **Pipeline-stage trace ORDER is best-effort**: the set of trace lines matches bash, but in-process (builtin/function) stages trace from a forked child while external stages trace from the parent, so the left-to-right ORDER of trace lines in a mixed pipeline may differ from bash (the lines themselves are identical).
