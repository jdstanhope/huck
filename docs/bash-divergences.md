...
- **L-57: `herestr` category residual — `declare -p` ANSI-C control-char value quoting (+ a harness-masked empty-command-name bug)** — `[deferred]`, low (re-scoped v219). The original L-57 — quote-provenance loss in `declare -f` / `type` reconstruction — was **RESOLVED in v219**: the lexer now wraps quoted runs in `WordPart::Quoted`, which carries the source quote span through generate/reconstruct, so quoted words re-render byte-identically to bash. That flipped the `cprint` bash-test-suite category to PASS (0 diff) and removed the provenance hunks from `herestr` (the `"$a $b"`, `'what a fabulous window treatment'`, and `'double"quote'` function-body lines now match). Two unrelated blockers, measured 2026-06-25, keep `herestr` FAIL: (1) **`declare -p` ANSI-C control-char VALUE quoting** — the SOLE residual hunk under the authoritative runner. An indexed-array element whose value contains a non-printable byte (here an embedded newline) is rendered by huck inside ordinary double quotes with the literal control byte, where bash uses the ANSI-C `$'…'` escape form (`[3]=$'i\n'`). Same char-vs-escape family as L-11 / L-17(c); the value round-trips through `eval`/`source` either way. Fix: the `declare -p` value renderer must switch to `$'…'` when the value holds a non-printable byte. (2) **empty-command-name runtime bug (harness-masked)** — a command whose LEADING word expands to empty (`$Z echo hi` with `Z` unset, or the test's `${THIS_SH} ./herestr1.sub` when `THIS_SH` is unset) should drop the empty word via word-splitting and run the remainder; huck instead treats the empty expansion as an empty command NAME and prints `huck: command not found:` (rc 127). This surfaces only on a DIRECT `huck herestr.tests` invocation; the runner exports `THIS_SH=$HUCK`, so under the runner it does NOT gate the category (the sub-script runs and its `foo`/`qux` output matches). Still a genuine general divergence worth fixing on its own. Both are out of scope for v219's reconstruction work.
...
- **L-59: residual `declare -f` / `type` reconstruction-fidelity gaps** — `[deferred]`, low (v218). The v218 port made function reconstruction byte-identical to bash for the common constructs (verified by `declare_f_diff_check.sh`), but two narrower fidelity gaps remain, surfaced by the `arith-for`/`func` bash test categories: (a) huck leaves an **empty `for ((` section** empty (`for ((; i<3; i++))`) where bash normalizes a missing section to `1` (`for ((1; i<3; i++))`) — `arith_for_to_source` would need to emit `1` for a `None` section; (b) a **brace group carrying a redirect inside a function body** (`{ …; } 1>&2`) reconstructs with the redirect migrated to the outer function close-brace rather than kept on the inner group — `FunctionDef`'s body-unwrap drops the brace-group wrapper when the group has its own redirects (bash keeps it). Both are reconstruction-only (no runtime effect). Each blocks full PASS of its category in combination with that category's other (non-reconstruction) blockers — arith-for also has arith-error-message wording divergences; func also has `function`-keyword preservation, `FUNCNAME` population, and `FUNCNEST` default (100 vs 200) gaps.
