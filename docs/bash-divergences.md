...
- **L-44: associative-array `declare -p` / bare-`declare` serialization format** — `[deferred]`, low (found in the v157 runtime sweep, batch 4; bare-`declare` form added v190). For an associative array huck emits `declare -A m=(["k"]="v" ["x"]="y")` whereas bash emits `declare -A m=([x]="y" [k]="v" )` — three differences: (a) huck quotes the subscript key (`["k"]`) where bash leaves a bareword key unquoted (`[k]`, quoting only keys that need it); (b) bash appends a trailing space before the closing `)`; (c) bash lists elements in its internal hash order, huck in insertion order. v190's bare `declare` (no args) reuses the same associative-array value renderer (`render_declare_value_part`), so the bare `m=(["k"]="v")` listing form inherits the identical (a)/(b)/(c) divergence — scalars and indexed arrays in the bare form are byte-identical to bash. Both forms round-trip correctly through `eval`/`source` (the value is the point), so this is cosmetic. (c) is impractical to match — bash's hash order is an implementation detail and huck's insertion order is arguably more useful; (a)/(b) are matchable in the shared renderer if exact byte-parity is ever wanted. Related: L-41 (`declare -p` omits computed dynamics).
...
- **L-54: quoted glob metachar in a `${var/pat/repl}` pattern, and a quoted number in `${var:off:len}`** — `[deferred]`, low (found during v200). Two residual operand-quote gaps NOT covered by v200's value-operand fix: (a) a quoted glob metacharacter in a substitution/remove PATTERN is not matched literally — `x=star; ${x/'*'/_}` → bash keeps `*` literal (no match → `star`), huck globs it (`_`). This is the PATTERN analogue of v199's L-23 regex fix: the substitute/remove matcher expands the pattern Word to a string and loses the per-part `quoted` flag (it should `escape_pattern_literal` quoted spans, like `expand_pattern` does for `case`). Diverges in BOTH enclosing-quote contexts, so it is independent of v200/M-15b. (b) a quoted number in an arithmetic substring offset/length — `${x:1:'2'}` — is accepted by huck (quotes stripped → `2`) but bash ERRORS (`'2': syntax error: operand expected`) because inside the arith context the quotes are literal and `'2'` is not a number. Both rare; (a) is the more real-world one (a literal `*`/`?` in a `${x/…}` replacement).
