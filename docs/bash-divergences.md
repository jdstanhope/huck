...
- **L-69: residual `expand_aliases` (non-interactive) edges (post-v231)** — `[deferred]`, low (v231). v231 wired the existing alias expander into the non-interactive file/`source` seam (`run_sourced_contents_in_sinks`), honoring `shopt expand_aliases` in scripts via a provenance map (offsets stay anchored to the original source) + an `alias_generation` re-tokenize for def-then-use timing. This is a **major shrink** of the `alias` bash-test category but did NOT flip it; four residuals remain: (a) **redirect-prefix command position** — a command word that follows a redirection (`< file foo`, `>out cmd`) is NOT alias-expanded, because the token-level eligibility in `alias_expand::process_token_mapped` treats a redirect operator as ending command position; bash expands it. Pre-existing in the expander (v231 only renamed/mapped it); a bash-exact fix needs redirect-aware eligibility or grammar-level expansion. (b) **`-c` command-string mode is not wired** — `huck -c 'shopt -s expand_aliases; alias x=…; x'` does not expand, because the `-c` path (`run_cmdstring`) is separate from `run_sourced_contents_in_sinks`; the runner gates the `alias` category through file mode, so this is not category-blocking but is a real gap. (c) an **alias defined AND first used within the same logical unit** (`alias x=…; x` on one physical line / one compound) is not expanded — documented edge-limit; a bash-faithful fix needs a push-back lexer. (d) two test-specific residuals in the `alias` category (`alias2.sub` `$TMPDIR`/permission infra, `alias4.sub` unterminated-quote parse) are unrelated to expansion. The common def-on-one-line-use-on-a-later-line patterns, trailing-space continuation, redefine, and unalias are byte-identical to bash (`alias_expand_diff_check.sh` 7/7), and `set -v` echoes the RAW pre-expansion line. v231 also fixed `source` path resolution (no-slash `$PATH`+CWD fallback gated on `shopt sourcepath`) and device-file acceptance (`/dev/null`, `/dev/stdin`, fifos, procsub `/dev/fd/N` — was rejected by `Path::is_file()`); both are byte-identical to bash (`source_device_diff_check.sh` 6/6) and shrink `builtins` (which keeps ~10 other blockers — no flip).
