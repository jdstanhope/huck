...
- **L-59: residual `declare -f` / `type` reconstruction-fidelity gaps** — `[deferred]`, low (v218). The v218 port made function reconstruction byte-identical to bash for the common constructs (verified by `declare_f_diff_check.sh`), but two narrower fidelity gaps remain, surfaced by the `arith-for`/`func` bash test categories: (a) huck leaves an **empty `for ((` section** empty (`for ((; i<3; i++))`) where bash normalizes a missing section to `1` (`for ((1; i<3; i++))`) — `arith_for_to_source` would need to emit `1` for a `None` section; (b) a **brace group carrying a redirect inside a function body** (`{ …; } 1>&2`) reconstructs with the redirect migrated to the outer function close-brace rather than kept on the inner group — `FunctionDef`'s body-unwrap drops the brace-group wrapper when the group has its own redirects (bash keeps it). Both are reconstruction-only (no runtime effect). Each blocks full PASS of its category in combination with that category's other (non-reconstruction) blockers — arith-for also has arith-error-message wording divergences; func's full re-measured residual is catalogued in **L-61** (this entry's (b) brace-group-redirect gap is one of func's four remaining blockers).

- **L-61: `func` bash-test category — re-measured residual (v221)** — `[deferred]`, medium. The `func` category was fully re-measured in v221: its complete residual is FOUR independent blockers (one prior blocker — prefix assignments leaking across a function call — was FIXED in v221, see git history; the `5 30`→`5 20` posix case and the `AVAR` leak hunks are gone). Remaining, none individually flips the category: (a) **`declare -xF`/`declare -xf` ignore the export filter** — with no exported functions, bash prints nothing but huck lists every function (`declare -f a`, `declare -f b`, …); the `-x` (exported-only) restriction is not applied to the function-listing path. (b) **`FUNCNAME` assignment is not protected** — `FUNCNAME=7` outside a function takes effect in huck (`echo $FUNCNAME` → `7`), whereas bash keeps `FUNCNAME` empty outside functions and ignores the user assignment; huck's call-stack population of `FUNCNAME` *inside* functions is otherwise correct. (c) **`declare -f` reconstruction of a redirected brace-group function body** — the L-59(b) gap (`{ …; } 1>&2` body) plus **`function`-keyword non-preservation** (bash keeps the `function` keyword when the source used it, e.g. nested `function f3 () …`; huck always drops it — needs a parser flag recording keyword-vs-paren form). (d) **`FUNCNEST` is not enforced** — huck ignores the `FUNCNEST` variable's recursion-depth limit entirely; worse, deep unbounded recursion overflows the Rust call stack and aborts the process (`fatal runtime error: stack overflow`) instead of bash's graceful `maximum function nesting level exceeded (N)` + return. The stack-overflow is a real robustness bug independent of the category. Fixing all four would flip `func` to PASS.
