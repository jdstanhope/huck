...
- **L-61: `func` bash-test category — re-measured residual (v221, updated v222)** — `[deferred]`, medium. The `func` category was fully re-measured in v221; two of the original blockers have since been fixed (v221: prefix assignments leaking across a function call — the `5 30`→`5 20` posix case + the `AVAR` leak; v222: the redirected-brace-body + nested-`function`-keyword reconstruction gaps). THREE independent blockers remain, none of which individually flips the category: (a) **`declare -xF`/`declare -xf` ignore the export filter** — with no exported functions, bash prints nothing but huck lists every function (`declare -f a`, `declare -f b`, …); the `-x` (exported-only) restriction is not applied to the function-listing path. (b) **`FUNCNAME` assignment is not protected** — `FUNCNAME=7` outside a function takes effect in huck (`echo $FUNCNAME` → `7`), whereas bash keeps `FUNCNAME` empty outside functions and ignores the user assignment; huck's call-stack population of `FUNCNAME` *inside* functions is otherwise correct. (c) **`FUNCNEST` is not enforced** — huck ignores the `FUNCNEST` variable's recursion-depth limit entirely; worse, deep unbounded recursion overflows the Rust call stack and aborts the process (`fatal runtime error: stack overflow`) instead of bash's graceful `maximum function nesting level exceeded (N)` + return. The stack-overflow is a real robustness bug independent of the category. Fixing all three would flip `func` to PASS.
