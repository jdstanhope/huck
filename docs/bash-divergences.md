...
| Low-impact (Tier 4) | 42 | Open edge cases / cosmetic divergences (`[low]`/`[intentional]`/`[deferred]`). |
...
- **L-51: a pipeline gets its own process group even when job control is OFF** — `[deferred]`, low (found 2026-06-16 during the orphaned-process investigation). huck unconditionally `setpgid`s the first stage of every multi-stage pipeline into a NEW process group (`executor.rs` ~line 2466 `setpgid(pid, pid)`, with later stages joining it via the child `setpgid(0, pgid_target)`), so a pipeline runs in its own group (`pgid` = first-stage pid) even in a NON-interactive shell (script / piped stdin) where job control is off. bash only puts a pipeline in its own group when job control is ENABLED (interactive, or `set -m`); a non-interactive bash keeps the pipeline's children in the SHELL's own process group. Consequence: a group-directed signal or cleanup aimed at the shell's process group (`kill -- -$$`, or a parent killing huck's process group) does NOT reach huck's non-interactive pipeline children — they sit in a sibling group, and because Unix reparents (rather than kills) orphans, they linger if the shell dies without reaping them. This is the huck-side root cause behind the test-harness orphan leak fixed 2026-06-16 (the `run_guarded` watchdog had to walk the descendant tree via `pgrep -P` because a single group-kill could not reach the pipeline). Normal execution and output are byte-identical to bash; only group-signal/cleanup semantics differ, and only for non-interactive pipelines. Fix: gate the pipeline `setpgid` (the first-stage `setpgid(pid, pid)` + the child `setpgid(0, pgid_target)`) on job control being enabled, mirroring bash — but this touches the job-control machinery (v108 `in_subshell` / v121 `in_completion` / v124 tty-deadlock fixes), so it needs care to avoid reintroducing a subshell/pipeline terminal-handoff deadlock.
