...
- **L-51: `scan_arith_block` mismatches `((` against a distant `))` in deeply-nested subshell pipelines** — `[deferred]`, low (found during v184; pre-existing). v184 made `((` at command position fall back to nested subshells `( (` when `scan_arith_block` finds no matching `))`. But `scan_arith_block` (`src/lexer.rs:1994`) tracks only raw `(`/`)` depth — it is BLIND to quotes, `$(…)`, `$((…))`, and backticks, and does not bail on shell metacharacters (`;`/`|`/newline) that can't appear in arithmetic. So a deeply-nested subshell pipeline like kselftest `runner.sh` line 128 (`((((( tap_timeout "$cmd" … >&3) | … ) | (read xs; …)) 4>>"$logfile" && …`) plus a LATER `$(( test_num + 1 ))` makes the scanner wrongly match the line-128 `((` against that distant `))`, return a bogus `Ok(body)` (with an unbalanced `$(`), and fail downstream as "unterminated command substitution" — so the v184 `Err`→subshell fallback never fires. zdiff (the simpler `((`-no-`))` case) IS fixed by v184; only the distant-`))` mismatch remains. A correct fix needs a quote/`$()`/backtick-aware `scan_arith_block` (or bash-accurate arith-vs-subshell parsing that keeps `((echo hi))` an arith error while routing line-128 to subshells) — substantial and touches every `((…))` arith block, so deferred. Affects 2 corpus scripts (runner.sh ×2 header copies).
