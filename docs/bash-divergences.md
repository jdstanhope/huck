...
- **L-61: `func` bash-test category — last residual blocker is a posix temp-assignment/special-builtin interaction** — `[deferred]`, medium. The `func` category's original blockers have been cleared across v221–v224 (v221: prefix-assignment leak; v222: redirected-brace-body + nested-`function`-keyword reconstruction; v223: `declare -xF` export filter/format + FUNCNAME write protection; v224: FUNCNEST enforcement + recursion backstop — func4.sub now byte-identical). ONE hunk remains (func3.sub line 155) — fixing it flips `func` to PASS: a **prefix assignment whose function persists the same variable via a special builtin in posix mode** is wrongly restored. `set -o posix; var=0; func(){ var=20 return 5; }; var=30 func; echo $? $var` → bash `5 20`, huck `5 0`. v221 made prefix assignments snapshot/restore around a function call, but bash's prefix assignment is a *shadow*: the function's special-builtin write (`var=20 return` — which persists in posix) reaches the global and SURVIVES, whereas huck's snapshot/restore overwrites it back to the pre-command value. The fix must skip restoring a prefix-assigned var that the function persisted via a special-builtin assignment (a v221 `restore_inline_assignments` follow-on; scheduled for v225).
