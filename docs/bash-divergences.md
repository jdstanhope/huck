...
- **L-48: integer `+=` concatenates-then-coerces instead of arithmetic-add** — `[deferred]`, low (pre-existing, found during v160 review). For an integer-flagged scalar, bash's `+=` is ARITHMETIC addition: `s=5; declare -i s; s+=3; echo $s` → bash `8`. huck concatenates the RHS then integer-coerces the result → `53`. The `+=` scalar-append path reads the existing value, string-concatenates the RHS, and only then applies the integer attribute via `value_with_scalar_attrs`; for an integer var the append should instead evaluate `existing + RHS` arithmetically. Pre-existing (reproduces on pre-v160 main); independent of namerefs. Fix: in the scalar `+=` path, when the target is integer-flagged, arith-evaluate `current_value + RHS` rather than concatenating. Plain (non-integer) `+=` and all array/assoc appends are unaffected.
