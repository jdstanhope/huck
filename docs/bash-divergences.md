...
- **L-49: `declare -ai` / `declare -Ai` integer arrays are unimplemented** — `[deferred]`, low (pre-existing; surfaced fixing L-48). Declaring an array with the integer attribute (`declare -ai a=(…)` indexed, `declare -Ai m=(…)` associative) errors `huck: declare: integer arrays not yet supported` and the variable is not created. bash arith-evaluates every element on assignment (`declare -ai a=(2+3 4*5)` → `a=(5 20)`). The scalar integer attribute (`declare -i`) IS supported, including arithmetic `+=` on a scalar or on an element of an array that became integer via `declare -i a; a[i]=…` (L-48, fixed). Only the dedicated integer-ARRAY declaration forms are rejected; the guard is explicit in `builtin_declare_decl`/`builtin_local_decl`. Adding them means routing each array element's value through `eval_integer_coerce` in the array-literal and element-assign paths and preserving the integer flag across `replace_array`. Rare in practice.
