...
- **L-70: DORMANT parser-driven backtick decode miscounts backslash runs ≥ 4** — `[deferred]`, low (v245). NOT a live/user-visible divergence: this is internal Phase-C debt in the DORMANT parser-driven front-end (reached only by the differential tests, never by the production lex→parse path). v245 inverted `` `…` `` command substitution — including arbitrary-depth escaping-based nesting — into the new stacked lexer/parser (`Mode::Backtick`, `Begin/EndBacktick` atoms; all byte-identical to the production oracle for the shipped corpus). The residual: `scan_step_backtick`'s body `\`-run decode consumes backslashes two-at-a-time incrementally, whereas the production oracle collapses the WHOLE contiguous run first (backtick unescape `\\`→`\`, `\$`→`$`, `` \` ``→`` ` ``) and THEN re-lexes the survivors as a command. The two agree for runs of 1–3 backslashes (the `bt_body_content` corpus) but diverge for runs ≥ 4 and for an ODD run immediately before `$`/`` ` ``. Worst case `` `echo \\\$x` ``: the new path decodes `$x` to a variable expansion while the oracle keeps it literal (`Quoted{Backslash,$}` + `x`). Pinned by `bt_backslash_run_divergence_deferred` (asserts the new≠old inequality so a future fix trips the test) and `bt_malformed_divergence_deferred` (the separate bare-`` ` ``-at-depth≥2 malformed-input lenient-accept). Zero production impact today; must be reconciled (make the new path re-implement the compounding two-pass escape semantics with a full parity matrix) before Stage-2 live-wires the parser-driven front end. Tracked as a dedicated follow-on iteration.
