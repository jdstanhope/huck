...
- **L-54: a quoted number in `${var:off:len}` substring arithmetic** — `[deferred]`, low (found during v200; the pattern-quoting part (a) was RESOLVED v201). A quoted number in an arithmetic substring offset/length — `${x:1:'2'}` — is accepted by huck (quotes stripped → `2`) but bash ERRORS (`'2': syntax error: operand expected`) because inside the arith context the quotes are literal and `'2'` is not a number. Rare. (The original L-54(a) — a quoted glob metacharacter in a `${x#pat}`/`${x%pat}`/`${x/pat/repl}` PATTERN not matching literally — was fixed in v201 by routing the pattern through `expand_pattern`, the same quoted-span escaping `case`/`[[ == ]]` use.)
