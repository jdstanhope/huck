...
- **L-53: background (`&`) pipelines/subshells get their own process group when job control is OFF** — `[deferred]`, low (v172 follow-on). v172 fixed the FOREGROUND case (former L-51): non-interactive foreground pipelines, `( … )` subshells, and single external commands now stay in the shell's process group via `Shell::job_control_active()` (`is_interactive && !in_subshell && !in_completion`) + the `NO_PGROUP` (-1) `pgid_target` sentinel that makes the fork primitives (`fork_and_run_in_subshell` / `spawn_external_with_fds`) skip `setpgid` entirely. The `run_background_sequence` path (`(…) &`, `a | b &`) still `setpgid`s its first stage into a new group unconditionally (`executor.rs` ~2045/2367, gated only on `first_pid.is_none()`), so a non-interactive backgrounded pipeline is still in a sibling group. Deferred because background pgroup semantics are subtler and `run_background_sequence` has its own hardcoded `setpgid` machinery distinct from the fork primitives; the same `job_control_active()`/`NO_PGROUP` treatment applies when revisited.
