...
- **L-40: `BASH_SOURCE` / `BASH_LINENO` call-stack arrays** — `[deferred]`, low (v151 follow-on). v151 added the dynamic `FUNCNAME` array (M-107). bash keeps two companion arrays in lockstep with `FUNCNAME`: `BASH_SOURCE[i]` = the source file where frame `i`'s function was defined, and `BASH_LINENO[i]` = the line number from which frame `i` was called. huck implements neither. `BASH_SOURCE` needs per-function definition-site (source file) tracking; `BASH_LINENO` needs the call-site line number — now feasible since **v152 added the `LINENO` machinery** (`ExecCommand.line` + `Shell.current_lineno`), which a `call_function`-time capture can read (parallel to the v151 `FUNCNAME` stack). Both are now unblocked (planned as v153). Rarely used outside error-reporting frameworks; the `FUNCNAME` surface (the common case) is covered.
