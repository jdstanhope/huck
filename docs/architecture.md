...
| `executor.rs` | Walks `Command` / `Sequence` / `Pipeline` trees. `run_*` functions per AST shape. `apply_one_assignment` + `apply_inline_assignments` + `restore_inline_assignments`. Pipeline fork/exec; subshell forking for compound stages. ERR-trap + errexit wire-in via `maybe_errexit`. v355 (#480): suppression is TWO counters on `Shell` — `errexit_suppressed_depth` and `err_trap_suppressed_depth`, raised via `suppress_both` / `suppress_errexit_only` and read via `errexit_suppressed()` / `err_trap_suppressed()`. Four contexts raise them: a non-last `&&`/`||` command, an `if`/`elif` condition, a `while`/`until` condition (all both), and a negated pipeline (errexit always; the ERR trap only when errexit is on — bash fires a negated compound's body trap with `set +e` and not with `set -e`). The exemption propagates INTO the command's body, which is what makes `set -e; f() { false; echo x; }; f || echo or` print `x` instead of exiting. Child stdio for forked stages/subshells is carried by `ChildStdio`/`ChildFd` (see `child_fd.rs`). |
