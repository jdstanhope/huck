...
- **`huck-engine`** (`crates/huck-engine/`) — the **terminal-free execution
  ...
  **rustyline-free** (a stray `use rustyline` here won't compile) — it is the
  ...
  constructs by construction. This fixed the genuine bash-parity gaps where the
  old scanner produced nothing (`for x in <word>` list position, and the
  command-substitution / redirect-target / path cases). It also makes huck
  complete *more* than bash in three contexts where bash's textual completer
  gives up — after `if`/`while`/`then`/`do`, inside a double-quoted `$(…)`, and
  in arithmetic `$(( … ))` — a kept-by-design improvement (see
  `docs/bash-divergences.md`, [#249](https://github.com/jdstanhope/huck/issues/249)).
...
| `shell_state.rs` | `Shell` struct (all session state) + `Variable` + `VarValue` (`Scalar`/`Indexed`/`Associative`) + `ShellOptions` + `AssignErr` / `DeclareErr`. Snapshot/restore primitives (`snapshot_var`/`restore_var`, `snapshot_for_local_scope`). Also `Suppression` (v356) — bash's ignore-return as shell state, with PRIVATE counts reached only through `Shell`'s `suppress_*` / `*_suppressed` methods; the privacy is load-bearing, since as bare fields beside the trap table they were swept into `clear_for_subshell` and the exemption died at every fork. |
...
| `executor.rs` | Walks `Command` / `Sequence` / `Pipeline` trees. `run_*` functions per AST shape. `apply_one_assignment` + `apply_inline_assignments` + `restore_inline_assignments`. Pipeline fork/exec; subshell forking for compound stages. ERR-trap + errexit wire-in via `maybe_errexit`. v355 (#480) + v356 (#1): suppression is bash's ignore-return, held in `Shell::suppression` (a `Suppression` in `shell_state.rs`) as TWO counts — errexit and ERR-trap — raised via `suppress_both` / `suppress_errexit_only` and read via `errexit_suppressed()` / `err_trap_suppressed()`. Four contexts raise them: a non-last `&&`/`||` command, an `if`/`elif` condition, a `while`/`until` condition (all both), and a negated pipeline (errexit always; the ERR trap only when errexit is on — bash fires a negated compound's body trap with `set +e` and not with `set -e`). The exemption propagates INTO the command's body, which is what makes `set -e; f() { false; echo x; }; f || echo or` print `x` instead of exiting — and ACROSS A FORK, which is why `set -e; ( false; echo x ) || echo or` prints `x`. `run_list_element` owns one element of an and-or list end to end — the exempt scope, the command, the interrupt checkpoint, control-flow propagation and the epilogue — so the scope covers the epilogue and `finish_command` decides from suppression state rather than a separate `is_last` flag. v364 (#676/#685) adds the other half of the rule: ONE ADJUDICATION PER FAILURE, at the site that PRODUCED it. `status_produced_by_body` names the kinds whose status is INHERITED from a body that was already adjudicated (`if`/`while`/`for`/`ArithFor`/`Select`/`Case`/`BraceGroup`/`Redirected`, looking through a single-stage `!` wrapper), and BOTH consumers are gated on it — previously only the ERR trap was, so a compound re-judged its body's status for errexit and exited on an exemption it could not see. The reporter splits into `finish_command` (reads the answer off the kind), `finish_command_own` (a caller that KNOWS the status is its own) and `finish_command_inner` (acts on the answer); `run_redirected` uses the second, because a FAILED redirect means the inner never ran and nothing inside adjudicated anything. The four loop runners share `loop_body_step` (`LoopStep::{Next,Stop,Propagate}`) so this rule threads one place, not four. Child stdio for forked stages/subshells is carried by `ChildStdio`/`ChildFd` (see `child_fd.rs`). |
...
| `builtin_opts.rs` | v359 (#496): one option scanner shared by ~28 builtins, modelled on bash's `internal_getopt` — bundled shorts, `--` terminator, a lone `-` as an operand, attached/separate values, scanning that stops at the first non-option (no permutation). `Getopt::new(name, ArgView, spec) -> next_opt(shell, err) -> Result<Option<Opt>, i32>`; `ArgView::{Plain, Decl}` lets the four `DeclArg`-based declaration builtins share it with the `&[String]` ones. `usage_for(name) -> &'static str` is keyed on the **invoked** name (`readarray` vs `mapfile`, `typeset` vs `declare`), transcribed verbatim from bash 5.2.21. Not every getopt-shaped builtin is on it: `set` doesn't fit the contract (long options, `+`/`-` symmetry); `exec` still hand-rolls its own scan (#516); `bg`/`bind`/`dirs`/`fg`/`pushd`/`popd` were out of scope — their divergences are check-ordering (#518) and `+N`/`-N` numeric parsing (#519), not scanning. |
...
| New option on a builtin already using `builtin_opts` | Declare the character in that builtin's `Getopt` spec string (append `:` for a value-taking option), add the `match` arm on the returned `Opt.ch`, and add the flag to its usage string in `builtin_opts::usage_for` — keyed on the **invoked** name (`readarray` gets its own arm from `mapfile`'s). `+`-style options are NOT this scanner's job — bash's own `internal_getopt` doesn't handle `+` either, so `declare`/`local`/`compopt` keep their own `+` handling ahead of the scanner. **Never add a character to the spec that huck doesn't actually implement** — an accepted-but-ignored option parses clean and produces silently wrong results instead of a clean rejection (this cost a Critical regression during v359). `exec` is the one getopt-shaped builtin still hand-rolling its own scan outside this scanner (#516). |
