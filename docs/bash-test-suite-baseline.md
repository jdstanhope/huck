...
| arith-for | FAIL | The `declare -f` trailing-space format divergence is resolved by v218. **Both catalogued divergences are now resolved** (2026-08-12): the reconstruction gap — an empty `for ((` section printed empty where bash normalizes it to `1` — by #64, and the malformed-header error wording by #313 (bash names the malformation, `arithmetic expression required` or `` `;' unexpected ``, and echoes the header as typed). A flip is NOT implied: per the v340 note above, this category also carries a non-behavioral `$0` program-name artifact in `-c` error output (`huck:` vs `bash:`) that huck code cannot fix, and the quoted-`;`-in-header split (#602) is still open. Re-run the category to confirm its current status. |
