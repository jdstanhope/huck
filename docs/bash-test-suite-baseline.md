...
| arith-for | FAIL | The `declare -f` trailing-space format divergence is resolved by v218. Remaining divergences: empty `for ((` sections are reconstructed as `1` rather than left empty (`for ((; i<3; i++))` reconstructs as `for ((1; i < 3; i++ ))`); error-message wording for malformed `for ((` headers (wrong section count or a quoted string as a section value) still differs between huck and bash. |
