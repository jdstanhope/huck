...
A continuation line — the part you type after the `>` prompt of a multi-line
`if` — is barely coloured, because it is parsed on its own and `then echo hi` is
not a valid command by itself. Highlighting the whole accumulated command is
[#670](https://github.com/jdstanhope/huck/issues/670). Colours are not
configurable yet either — the palette is fixed
([#667](https://github.com/jdstanhope/huck/issues/667)). Bracket globs like
