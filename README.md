...
| v122      | **`BASH_REMATCH` array population (M-14 sub-feature)** — `[[ … =~ REGEX ]]` matched but never populated the `BASH_REMATCH` array, so `_longopt`-style completers (bash-completion extracts an option name via `[[ $line =~ (--[a-z-]+) ]] && … ${BASH_REMATCH[1]}`) got no candidates and `ls -<TAB>` offered nothing. Fix: the Regex arm switched from `re.is_match` to `re.captures` — on a match it builds an indexed array (`BASH_REMATCH[0]`=whole match, `[1..]`=capture groups, non-participating group→`""`); on a non-match it clears the array (matching bash, which resets it on every `=~`). **PAYOFF: `ls -<TAB>`/`ls --a<TAB><TAB>` now offers `--all`/`--almost-all`/etc. via `_longopt`'s `BASH_REMATCH` extraction — verified responsive under a PTY against real bash-completion.** Byte-identical to bash across whole+groups, no-match-clears, non-participating groups, substring, quoted, anchored, version-digit capture, `_longopt` extraction, and `${!BASH_REMATCH[@]}` indices. (mise candidates still need the bash-completion 2.12 env API — environmental.) 45th harness `bash_rematch_diff_check.sh` (9 fragments) + integration tests; full suite 2888 tests pass, clippy clean |
